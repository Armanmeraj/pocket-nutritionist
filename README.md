...
- **Nutrition database lookup** — text/photo/voice results can be resolved against saved confirmed matches, Open Food Facts text search, and optional USDA FoodData Central BYOK lookup before falling back to the AI estimate
...
- **Source badges** — reviewed and saved meals show whether nutrition came from a saved match, Open Food Facts, USDA FoodData Central, AI estimate, or manual entry
...
Food logging also supports exact nutrition lookup before final review. The app can reuse your previously confirmed matches, query Open Food Facts by barcode or food name, and query USDA FoodData Central when you add your own USDA API key in Settings. If no confident database match is found, the AI estimate remains available and clearly labeled.
...
```
Photo(s) / Text / Voice / Barcode / Database Search
        │
        ▼
  Saved confirmed matches + nutrition databases when available
        │
        ├── Open Food Facts barcode/text lookup
        ├── USDA FoodData Central lookup if you add a USDA key
        └── BYOK provider API if no confident database match is found
        ...
        │
        ▼
  Reviewed nutrition with source badge
        │
        ▼
  User reviews & edits
        │
