...
enum OpenFoodFactsService {
    ...
    private static func analysis(
        ...
        let brand = brandName(from: product)
        ...
        let sourceMetadata = NutritionSourceMetadata(
            ...
            householdServingDescription: firstNonEmpty(product.servingSize),
            portionQuantity: 1,
            portionUnit: "serving",
    ...
    private static func brandName(from product: OpenFoodFactsProduct) -> String? {
        let brand = product.brands?
            .split(separator: ",")
            .first
            .map { String($0).trimmingCharacters(in: .whitespacesAndNewlines) }
        return firstNonEmpty(brand)
    }
