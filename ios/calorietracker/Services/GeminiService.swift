...
struct GeminiService {
    struct FoodAnalysis {
        ...
        var structuredItems: [StructuredFoodItem] = []
        var nutritionDataSource: NutritionDataSource = .aiEstimate
        var nutritionSourceMetadata: NutritionSourceMetadata? = nil
    }
    ...
    private static let structuredItemsInstruction = """
    structured_items is required. It is used only for database matching. Return each distinct food once using this exact object shape: {"name":"...","brand":null,"quantity":0.0,"unit":"g","branded":false,"variant":null,"preparation":null,"modifiers":[],"portion_estimated":false}. Preserve stated brands, product variants, preparation words like raw/cooked, and user-visible quantities/units. Use null for unknown brand/quantity/unit/variant/preparation and true for portion_estimated when the amount is guessed.
    """
    ...
    static func analyzeTextInput(description: String) async throws -> FoodAnalysis {
        ...
        \(Self.structuredItemsInstruction)
    ...
    static func parseFoodAnalysis(from text: String) throws -> FoodAnalysis {
        ...
        let structuredItems = parseStructuredItems(from: json)
        return FoodAnalysis(
            ...
            nutritionDataSource: .aiEstimate
        )
    }

    private static func parseStructuredItems(from json: [String: Any]) -> [StructuredFoodItem] {
        (json["structured_items"] as? [[String: Any]] ?? [])
            .prefix(20)
            .compactMap { item in
                guard let name = (item["name"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines),
                      !name.isEmpty
                ...
                return StructuredFoodItem(
                    name: name,
                    brand: (item["brand"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).foodAnalysisNonEmpty,
                    quantity: (item["quantity"] as? NSNumber)?.doubleValue,
                    unit: (item["unit"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).foodAnalysisNonEmpty,
                    branded: item["branded"] as? Bool,
                    variant: (item["variant"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).foodAnalysisNonEmpty,
                    preparation: (item["preparation"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).foodAnalysisNonEmpty,
                    modifiers: (item["modifiers"] as? [String] ?? [])
                        .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                        .filter { !$0.isEmpty },
                    portionIsEstimated: item["portion_estimated"] as? Bool ?? false
                )
            }
    }
...
private extension String {
    var foodAnalysisNonEmpty: String? {
        isEmpty ? nil : self
    }
}
