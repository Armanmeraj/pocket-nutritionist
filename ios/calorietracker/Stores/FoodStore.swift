...
class FoodStore {
    ...
    func reprocessEntry(_ entry: FoodEntry, withNote note: String) async throws -> GeminiService.FoodAnalysis {
        ...
        let resolution = await NutritionLookupCoordinator
            .live(previousEntries: entries)
            .resolve(analysis: result, originalText: description)
        switch resolution {
        case .resolved(let resolved), .noMatch(let resolved):
            return resolved
        case .ambiguous(_, _, let fallback):
            return fallback
        }
    }
