...
struct FoodResultView: View {
    ...
    let nutritionDataSource: NutritionDataSource
    let nutritionSourceMetadata: NutritionSourceMetadata?
    ...
    init(
        ...
        nutritionSourceMetadata: NutritionSourceMetadata? = nil,
        ...
        self.nutritionDataSource = nutritionDataSource ?? NutritionDataSource.defaultValue(for: source)
        self.nutritionSourceMetadata = nutritionSourceMetadata
                ...
                List {
                    ...
                    Section("Food Details") {
                        ...
                        NutritionSourceRow(
                            ...
                            metadata: currentNutritionSourceMetadata
                        )
                    }
    ...
    private func makeFoodEntry(includeImage: Bool) -> FoodEntry {
        FoodEntry(
            ...
            nutritionSourceMetadata: currentNutritionSourceMetadata
        )
    }

    private var currentNutritionSourceMetadata: NutritionSourceMetadata? {
        guard var metadata = nutritionSourceMetadata else { return nil }
        metadata.portionQuantity = selectedServingQuantity
        metadata.portionUnit = selectedServingOption.unit
        metadata.portionIsEstimated = metadata.portionIsEstimated || !servingSizeIsKnown
        if servingSizeIsKnown {
            metadata.servingWeightGrams = servingSizeGrams
        }
        return metadata
    }
...
struct NutritionSourceRow: View {
    ...
    private var detailText: String? {
        var parts: [String] = []
        if let brand = metadata?.matchedBrand, !brand.isEmpty {
            parts.append(brand)
        }
        if let portion = metadata?.portionSummary {
            parts.append(portion)
        }
        if let confidence = metadata?.matchConfidence, confidence < 0.999 {
    ...
    private var foreground: Color {
        switch source {
        case .personalVerified: AppColors.protein
        case .openFoodFacts: AppColors.carbs
        case .usdaFoodDataCentral: AppColors.calorie
        case .aiEstimate: .secondary
        case .manual: AppColors.fat
        case .mixed: .primary
        }
    }
