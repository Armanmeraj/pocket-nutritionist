...
private struct NutritionCandidateSelection: Identifiable {
    let id = UUID()
...
struct HomeView: View {
    ...
    @State private var showDatabaseSearchPopover = false
    ...
    enum ActiveSheet: String, Identifiable {
        case analyzing, foodResult, analyzingText, lookingUpBarcode, nutritionMatches, editFood, importSharedMeal
        var id: String { rawValue }
    }
    @State private var activeSheet: ActiveSheet?
    @State private var editingEntry: FoodEntry?
    @State private var pendingSharedMeals: [FoodEntry] = []
    @State private var nutritionCandidateSelection: NutritionCandidateSelection?
    ...
    var body: some View {
        ...
        return NavigationStack {
            ...
            .overlay(alignment: .bottomTrailing) {
                Menu {
                    ...
                    Menu {
                        ...
                        Button {
                            presentFoodDestination {
                                showDatabaseSearchPopover = true
                            }
                        } label: {
                            Label("Database Search", systemImage: "magnifyingglass")
                        }
                    } label: {
                    ...
                    Menu {
                        ...
                        .popover(isPresented: $showDatabaseSearchPopover) {
                            NutritionDatabaseSearchView(
                                search: { query in
                                    await NutritionLookupCoordinator
                                        .live(previousEntries: foodStore.entries)
                                        .searchDatabase(query: query)
                                },
                                onCancel: {
                                    showDatabaseSearchPopover = false
                                },
                                onSelect: { analysis in
                                    showDatabaseSearchPopover = false
                                    currentImage = nil
                                    currentImages = []
                                    currentEmoji = analysis.emoji
                                    currentFoodSource = .manual
                                    currentFoodResult = analysis
            ...
            .sheet(item: $activeSheet) { sheet in
                ...
                case .foodResult:
                    if let result = currentFoodResult {
                        FoodResultView(
                            ...
                            nutritionDataSource: result.nutritionDataSource,
                            nutritionSourceMetadata: result.nutritionSourceMetadata,
                ...
                case .nutritionMatches:
                    if let selection = nutritionCandidateSelection {
                        NutritionCandidateSelectionView(
                            selection: selection,
                            onSelect: { candidate in
                                NutritionConfirmedMappingStore().remember(candidate, for: selection.item)
                                let result = candidate.scaledAnalysis(for: selection.item)
                                currentFoodResult = result
                                currentEmoji = result.emoji
                                nutritionCandidateSelection = nil
                                activeSheet = .foodResult
                            },
                            onUseEstimate: {
                                currentFoodResult = selection.fallback.withNutritionSource(.aiEstimate)
                                currentEmoji = selection.fallback.emoji
                                nutritionCandidateSelection = nil
                                activeSheet = .foodResult
                            },
                            onCancel: {
            ...
            .sheet(item: $savedMealsMode, content: { mode in
                RecentsView(mode: mode, logDate: logDateForSelectedDay, onReview: { entry in
                    ...
                    currentFoodResult = GeminiService.FoodAnalysis(
                        ...
                        nutritionDataSource: entry.nutritionDataSource,
                        nutritionSourceMetadata: entry.nutritionSourceMetadata
                    )
                    activeSheet = .foodResult
                })
            })
    ...
    private func presentQuickActionIfPossible() {
        ...
        let hadOpenDestination = showCamera || showBarcodeScanner || showPhotoPicker
            ...
            || showDatabaseSearchPopover || savedMealsMode != nil || showContextSheet || showMultiPhotoCaptureSheet
        ...
        Task {
            do {
                ...
                case .snapFood:
                    ...
                    retryRequest = nil
                    await resolveAndPresentFoodResult(
                        result,
                        source: .snapFood,
    ...
    private func resolveAndPresentFoodResult(
        _ analysis: GeminiService.FoodAnalysis,
        source: FoodSource,
        originalText: String?
    ) async {
        let resolution = await NutritionLookupCoordinator
            .live(previousEntries: foodStore.entries)
            .resolve(analysis: analysis, originalText: originalText)
        switch resolution {
        case .resolved(let result), .noMatch(let result):
            currentFoodResult = result
            currentEmoji = result.emoji
            currentFoodSource = source
            activeSheet = .foodResult
        case .ambiguous(let item, let candidates, let fallback):
            currentFoodResult = fallback
            currentEmoji = fallback.emoji
            currentFoodSource = source
            nutritionCandidateSelection = NutritionCandidateSelection(
                item: item,
                candidates: candidates,
                fallback: fallback
            )
            activeSheet = .nutritionMatches
        }
    }
...
private struct NutritionCandidateSelectionView: View {
    let selection: NutritionCandidateSelection
    let onSelect: (NutritionCandidate) -> Void
                ...
                Section {
                    ForEach(selection.candidates) { candidate in
                        Button {
                            ...
                            NutritionCandidateRow(candidate: candidate)
                        }
                        .buttonStyle(.plain)
                    }
                } header: {
                    ...
                    Text(selection.item.lookupQuery)
                }

                Section {
                    Button("Use AI Estimate", action: onUseEstimate)
                        .foregroundStyle(AppColors.calorie)
                }
            }
            .scrollContentBackground(.hidden)
            .background(AppColors.appBackground)
            .navigationTitle("Nutrition Match")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
...
private struct NutritionDatabaseSearchView: View {
    let search: (String) async -> [NutritionCandidate]
    let onCancel: () -> Void
    let onSelect: (GeminiService.FoodAnalysis) -> Void

    @State private var query = ""
    @State private var candidates: [NutritionCandidate] = []
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    HStack {
                        TextField("Food or barcode", text: $query)
                    ...
                    Section("Results") {
                        ForEach(candidates) { candidate in
                            Button {
                                ...
                                NutritionConfirmedMappingStore().remember(candidate, for: item)
                                onSelect(candidate.scaledAnalysis(for: item))
                            } label: {
                                NutritionCandidateRow(candidate: candidate)
                            }
                            .buttonStyle(.plain)
    ...
    private func runSearch() {
        ...
        hasSearched = true
        Task {
            candidates = await search(trimmed)
            isSearching = false
        }
    }
}

private struct NutritionCandidateRow: View {
    let candidate: NutritionCandidate
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Spacer()
                Text(candidate.nutritionDataSource.badgeText)
            ...
            HStack(spacing: 14) {
                Text("\(Int(round(candidate.facts.calories ?? 0))) kcal")
                Text("P \(MacroValueFormatter.string(candidate.facts.protein ?? 0))g")
                Text("C \(MacroValueFormatter.string(candidate.facts.carbs ?? 0))g")
                Text("F \(MacroValueFormatter.string(candidate.facts.fat ?? 0))g")
                Spacer()
                if candidate.matchScore > 0 {
                    Text("\(Int((candidate.matchScore * 100).rounded()))%")
                }
            }
    ...
    private var detail: String? {
        var parts: [String] = []
        if let serving = candidate.householdServingDescription, !serving.isEmpty {
            parts.append(serving)
        } else if let grams = candidate.servingWeightGrams {
            parts.append("\(NutritionNumberFormatter.quantity(grams)) g")
        }
        if let dataType = candidate.dataType, !dataType.isEmpty {
...
struct ProfileView: View {
    ...
    @AppStorage(FastingSettings.enabledKey) private var fastingTrackingEnabled = false
    @AppStorage(FastingSettings.defaultGoalMinutesKey) private var fastingDefaultGoalMinutes = FastingSettings.defaultGoalMinutes
    @AppStorage(NutritionLookupSettings.exactLookupEnabledKey) private var exactNutritionLookupEnabled = true
    @AppStorage(NutritionLookupSettings.openFoodFactsEnabledKey) private var openFoodFactsTextLookupEnabled = true
    ...
    @State private var usdaAPIKeyText: String = NutritionLookupSettings.usdaAPIKey ?? ""
    @State private var showUSDAAPIKey = false
    ...
    private var settingsList: some View {
            List {
                ...
                Section {
                        ...
                        Toggle(isOn: $exactNutritionLookupEnabled) {
                            Label {
                                Text("Use Exact Nutrition First")
                            } icon: {
                                Image(systemName: "checkmark.seal.fill")
                                    .foregroundStyle(AppColors.calorie)
                            }
                        ...
                        .tint(AppColors.calorie)
                        .onChange(of: exactNutritionLookupEnabled) { _, enabled in
                            NutritionLookupSettings.exactLookupEnabled = enabled
                        }

                        Toggle(isOn: $openFoodFactsTextLookupEnabled) {
                            Label {
                        ...
                        .tint(AppColors.calorie)
                        .onChange(of: openFoodFactsTextLookupEnabled) { _, enabled in
                            NutritionLookupSettings.openFoodFactsEnabled = enabled
                        }
                        ...
                        HStack {
                            ...
                            Group {
                                if showUSDAAPIKey {
                                    TextField("FoodData Central key", text: $usdaAPIKeyText)
                                } else {
                                    SecureField("FoodData Central key", text: $usdaAPIKeyText)
                                }
                            }
                            .textFieldStyle(.plain)
                            .multilineTextAlignment(.trailing)
                            .autocorrectionDisabled()
                            .textInputAutocapitalization(.never)
                            .onChange(of: usdaAPIKeyText) { _, newValue in
                                let trimmed = newValue.trimmingCharacters(in: .whitespacesAndNewlines)
                                NutritionLookupSettings.usdaAPIKey = trimmed.isEmpty ? nil : trimmed
                            }
                            Button {
                                showUSDAAPIKey.toggle()
                            } label: {
                                Image(systemName: showUSDAAPIKey ? "eye.fill" : "eye.slash.fill")
                                    .foregroundStyle(.secondary)
                                    .font(.system(size: 14))
                            }
                            .buttonStyle(.plain)
                    ...
                    Text("Food text and AI-identified items are matched against your saved verified meals, USDA FoodData Central when a key is set, and Open Food Facts when enabled. AI nutrition is used when no confident database match is available.")
                }
                .listRowBackground(AppColors.appCard)
                }
