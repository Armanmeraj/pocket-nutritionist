...
class KeyStore(context: Context) {
    ...
    fun usdaFoodDataCentralApiKey(): String? = load(USDA_FOOD_DATA_CENTRAL_API_KEY)

    fun setUsdaFoodDataCentralApiKey(key: String?) {
        val trimmed = key?.trim().orEmpty()
        if (trimmed.isEmpty()) delete(USDA_FOOD_DATA_CENTRAL_API_KEY) else save(USDA_FOOD_DATA_CENTRAL_API_KEY, trimmed)
    }
    ...
    companion object {
        ...
        private const val USDA_FOOD_DATA_CENTRAL_API_KEY = "nutrition_usda_food_data_central_api_key"
